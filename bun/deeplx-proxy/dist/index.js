...
app.post("/translate", async (c) => {
  ...
  const targetUrl = `https://deeplx.missuo.ru/translate?key=${encodeURIComponent(key)}`;
  const newHeaders = new Headers;
  newHeaders.set("Content-Type", "application/json");
  const jsonBody = JSON.stringify(body);
  const [error, res] = await safe((await fetch(targetUrl, {
    method: "POST",
    headers: newHeaders,
    body: jsonBody
  })).json());
