...
class TodoApp {
  ...
  private initializeElements(): void {
    ...
    this.clearCompletedButton = document.querySelector(
      ".clear-completed",
  ...
  private toggleFlag(id: string): void {
    const todo = this.todos.find(t => t.id === id);
    if (todo) {
  ...
  private getFilteredTodos(): Todo[] {
    ...
    switch (this.filter) {
      ...
      case "flagged":
        return this.todos.filter((t) => t.flagged);
      default:
        return this.todos;
    }
  }
  ...
  private render(): void {
    ...
    this.clearCompletedButton.style.display =
  ...
  private createTodoElement(todo: Todo): HTMLElement {
    ...
    todoItem.innerHTML = `
      ...
      <div class="todo-actions">
        <button class="action-button ${todo.flagged ? 'flagged' : ''}" aria-label="Flag todo">
    ...
    const flagButton = todoItem.querySelector(
    ...
    flagButton.addEventListener("click", () => this.toggleFlag(todo.id));
